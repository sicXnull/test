...
VERSION=
commit=false
url=https://github.com/reduxproject/redux
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the redux, gitian-builder, gitian.sigs, and redux-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/reduxproject/redux
...
then
	# Make output folder
	mkdir -p "./redux-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit redux=${COMMIT} --url redux=${url} ../redux/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../redux/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/redux-*.tar.gz build/out/src/redux-*.tar.gz ../redux-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit redux=${COMMIT} --url redux=${url} ../redux/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/redux-*.zip build/out/redux-*.exe ../redux-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit redux=${COMMIT} --url redux=${url} ../redux/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/redux-*.tar.gz build/out/redux-*.dmg ../redux-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit redux=${COMMIT} --url redux=${url} ../redux/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../redux/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/redux-*.tar.gz build/out/src/redux-*.tar.gz ../redux-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../redux/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/redux-*win64-setup.exe ../redux-binaries/${VERSION}
	    mv build/out/redux-*win32-setup.exe ../redux-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../redux/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/redux-osx-signed.dmg ../redux-binaries/${VERSION}/redux-${VERSION}-osx.dmg
	fi
	popd || exit
