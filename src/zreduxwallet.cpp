...
CzREDUXWallet::CzREDUXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zredux seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZREDUXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZREDUXSeed_deprecated()) {
                    LogPrintf("%s: Updated zREDUX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zredux seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zredux wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzREDUXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZREDUXCount(nCountLastUsed);
    else if (!walletdb.ReadZREDUXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzREDUXWallet::Lock()
{
    seedMaster = 0;
}

void CzREDUXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzREDUXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZREDUX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzREDUXWallet::LoadMintPoolFromDB()
{
...
void CzREDUXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzREDUXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzREDUXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zreduxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzREDUXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZREDUX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zreduxTracker which also adds to database
    pwalletMain->zreduxTracker->Add(dMint, true);
...
void CzREDUXWallet::SeedToZREDUX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzREDUXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzREDUXWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZREDUXCount(nCountLastUsed);
...
void CzREDUXWallet::GenerateDeterministicZREDUX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzREDUXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzREDUXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
