// Copyright (c) 2018 The REDUX developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zredux/deterministicmint.h>
#include "zreduxtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
#include "zredux/zreduxwallet.h"
#include "witness.h"

using namespace std;

CzREDUXTracker::CzREDUXTracker(std::string strWalletFile)
{
...
CzREDUXTracker::~CzREDUXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzREDUXTracker::Init()
{
...
bool CzREDUXTracker::Archive(CMintMeta& meta)
{
...
bool CzREDUXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzREDUXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzREDUXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzREDUXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzREDUXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzREDUXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzREDUXTracker::GetSerialHashes()
{
...
CAmount CzREDUXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzREDUXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzREDUXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzREDUXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzREDUXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzREDUXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzREDUXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzREDUXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzREDUXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzREDUXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzREDUXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzREDUXWallet* zREDUXWallet)
{
    bool iszREDUXWalletInitialized = (NULL != zREDUXWallet);
    CMintMeta meta;
    ...
    if (! iszREDUXWalletInitialized)
        zREDUXWallet = new CzREDUXWallet(strWalletFile);
    meta.isSeedCorrect = zREDUXWallet->CheckSeed(dMint);
    if (! iszREDUXWalletInitialized)
        delete zREDUXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzREDUXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzREDUXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzREDUXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzREDUXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzREDUXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzREDUXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzREDUXWallet* zREDUXWallet = new CzREDUXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zREDUXWallet);
        }
        delete zREDUXWallet;
        LogPrint("zero", "%s: added %d dzredux from DB\n", __func__, listDeterministicDB.size());
    }
