...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered redux address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 REDUX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal redux:URI (redux:XyZ...?message=example)
            {
...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passreduxase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passreduxase);
    } else {
...
bool WalletModel::changePassreduxase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassreduxase(oldPass, newPass);
    }
    return retval;
}
...
{
    setMints.clear();
    setMints = pwalletMain->zreduxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
