...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::REDUX);
    ...
    if (!settings.contains("nAnonymizeReduxAmount"))
        settings.setValue("nAnonymizeReduxAmount", 1000);

    nAnonymizeReduxAmount = settings.value("nAnonymizeReduxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeReduxAmount"))
        SoftSetArg("-anonymizereduxamount", settings.value("nAnonymizeReduxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in redux.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeReduxAmount:
            return QVariant(nAnonymizeReduxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeReduxAmount:
            nAnonymizeReduxAmount = value.toInt();
            settings.setValue("nAnonymizeReduxAmount", nAnonymizeReduxAmount);
            emit anonymizeReduxAmountChanged(nAnonymizeReduxAmount);
            break;
        case CoinControlFeatures:
