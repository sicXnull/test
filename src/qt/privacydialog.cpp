...
#include <QtWidgets>
#include <zredux/deterministicmint.h>
#include <zredux/accumulators.h>
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zREDUX ought to be enough for anybody." - Bill Gates, 2017
    ui->zREDUXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzREDUXSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zREDUXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzREDUX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREDUX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zREDUX, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zREDUX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zREDUX in ") +
...
void PrivacyDialog::on_pushButtonSpendzREDUX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREDUX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zREDUX, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zREDUX
        sendzREDUX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zREDUX
    sendzREDUX();
}

void PrivacyDialog::on_pushButtonZReduxControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZReduxControlDialog* zReduxControl = new ZReduxControlDialog(this);
    zReduxControl->setModel(walletModel);
    zReduxControl->exec();
}

void PrivacyDialog::setZReduxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzReduxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzREDUX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Redux Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zREDUXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zREDUX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " REDUX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zREDUX</b>";
    ...
    // use mints from zREDUX selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZReduxControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZReduxControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zREDUX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zREDUX"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zREDUX was spent successfully update the addressbook with the label
    ...
    // Clear zredux selector in case it was used
    ZReduxControlDialog::setSelectedMints.clear();
    ui->labelzReduxSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zREDUX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Redux, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zREDUX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zreduxTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zREDUX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zREDUX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zREDUX "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zREDUX "));
    ui->labelzREDUXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zREDUX </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zREDUX </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zREDUX </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzREDUXSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzREDUX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zREDUX
        ui->pushButtonMintzREDUX->setEnabled(false);
        ui->pushButtonMintzREDUX->setToolTip(tr("zREDUX is currently disabled due to maintenance."));

        // Spend zREDUX
        ui->pushButtonSpendzREDUX->setEnabled(false);
        ui->pushButtonSpendzREDUX->setToolTip(tr("zREDUX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zREDUX
        ui->pushButtonMintzREDUX->setEnabled(true);
        ui->pushButtonMintzREDUX->setToolTip(tr("PrivacyDialog", "Enter an amount of REDUX to convert to zREDUX", 0));

        // Spend zREDUX
        ui->pushButtonSpendzREDUX->setEnabled(true);
        ui->pushButtonSpendzREDUX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
