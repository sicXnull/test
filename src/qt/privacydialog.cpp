                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zREDUXpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
...
void PrivacyDialog::on_pushButtonMintzREDUX_clicked()
{
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passreduxase first."));
            return;
        }
    }
...
void PrivacyDialog::sendzREDUX()
{
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZREDUX_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zREDUX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zREDUX"));
            ui->TEMintStatus->repaint();
            return;
