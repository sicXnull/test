...
#include "ui_zreduxcontroldialog.h"
...
std::set<std::string> ZReduxControlDialog::setSelectedMints;
std::set<CMintMeta> ZReduxControlDialog::setMints;

bool CZReduxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZReduxControlDialog::COLUMN_DENOMINATION || column == ZReduxControlDialog::COLUMN_VERSION || column == ZReduxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZReduxControlDialog::ZReduxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZReduxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZReduxControlDialog::~ZReduxControlDialog()
{
    delete ui;
}

void ZReduxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZReduxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZReduxControlWidgetItem* itemDenom(new CZReduxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZReduxControlWidgetItem *itemMint = new CZReduxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            //LOCK(pwalletMain->zreduxTracker->cs_spendcache);

            //CoinWitnessData *witnessData = pwalletMain->zreduxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zREDUX seed used to mint this zREDUX is not the same as currently hold in the wallet";
            else
...
void ZReduxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZReduxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRedux_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZReduxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZReduxControlDialog::GetSelectedMints()
{
...
void ZReduxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
