...
#ifndef REDUX_ZREDUXWALLET_H
#define REDUX_ZREDUXWALLET_H
...
public:
    CzREDUXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZREDUX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZREDUX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //REDUX_ZREDUXWALLET_H
