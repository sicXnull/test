...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 REDUX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zREDUX send
enum ZerocoinSpendStatus {
    ZREDUX_SPEND_OKAY = 0,                            // No error
    ZREDUX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZREDUX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZREDUX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZREDUX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZREDUX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZREDUX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZREDUX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZREDUX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZREDUX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZREDUX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZREDUX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZREDUX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZREDUX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZREDUX_SPENT_USED_ZREDUX = 14,                      // Coin has already been spend
    ZREDUX_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZREDUX_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZREDUXOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzreduxAuto) const;
    ...
    */
    ...
    CzREDUXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzREDUXTracker> zreduxTracker;
    ...
    void setZWallet(CzREDUXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zreduxTracker = std::unique_ptr<CzREDUXTracker>(new CzREDUXTracker(strWalletFile));
    }

    CzREDUXWallet* getZWallet() { return zwalletMain; }
    ...
    bool Unlock(const SecureString& strWalletPassreduxase, bool anonimizeOnly = false);
    bool ChangeWalletPassreduxase(const SecureString& strOldWalletPassreduxase, const SecureString& strNewWalletPassreduxase);
    bool EncryptWallet(const SecureString& strWalletPassreduxase);
