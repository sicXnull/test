...
#if defined(NDEBUG)
#error "Redux cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uredux) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zREDUX is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zREDUX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zREDUX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zREDUX spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zREDUX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zREDUX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zredux");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zREDUX mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    if (nHeight == 0) {
        return 2500000 * COIN;
    } else if (nHeight > 0 && nHeight <= 400) {
        return 100 * COIN;
    } else if (nHeight > 400 && nHeight <= 553620) {
        return 2 * COIN;
    } else if (nHeight > 553620 && nHeight <= 1107140) {
        return 1.75 * COIN;
    } else if (nHeight > 1107140 && nHeight <= 2316276) {
        return 1.25 * COIN;
    } else if (nHeight > 2316276 && nHeight <= 5119808) {
        return 1 * COIN;
    } else if (nHeight > 5119808 && nHeight <= 10079616) {
        return 0.75 * COIN;
    } else if (nHeight > 10079616 && nHeight <= 30232165) {
        return 0.5 * COIN;
    } else if (nHeight > 30232165 && nHeight <= 393923045) {
        return 0.2 * COIN;
    } else {
        return 0.1 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Redux
...
{
    RenameThread("redux-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZREDUXMinted()
{
...
void RecalculateZREDUXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zREDUX supply
...
bool RecalculateREDUXSupply(int nHeightStart)
{
...
bool UpdateZREDUXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZREDUXSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zREDUXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert REDUX to zREDUX
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zREDUX spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zREDUX serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zREDUX mints and %d zREDUX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Redux Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Redux Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Redux Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Redux: We use certain sporks during IBD, so check to see if they are
