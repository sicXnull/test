...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zreduxTracker = unique_ptr<CzREDUXTracker>(new CzREDUXTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZREDUX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZREDUX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
